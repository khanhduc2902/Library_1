...
function Books(props) {
  ...
  const handleDelete = (id) => {
    const shouldDelete = window.confirm("Bạn có chắc chắn muốn xóa sách này không?");
    if (shouldDelete) {
      // Gửi yêu cầu xóa sách đến API
              ...
              {booksOnFilter.map((book) => (
                <tr key={book.id}>
                  ...
                  <td>
                    ...
                    <input type="button" value="Delete" className="btn btn-danger" onClick={() => handleDelete(book.id)} />
                  </td>
                </tr>
              ))}
            </tbody>
